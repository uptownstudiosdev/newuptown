...
// CAUSE WE CARE CUSTOM POST TYPE
function my_custom_post_cause() {
	$labels = array(
		'name'               => _x( 'Causes', 'post type general name' ),
		'singular_name'      => _x( 'Cause', 'post type singular name' ),
		'add_new'            => _x( 'Add New Item', 'cause' ),
		'add_new_item'       => __( 'Add New Item' ),
	...
	$args = array(
		'labels'        	     => $labels,
		'description'   	     => 'Holds our cause and cause specific data',
		'capablility_type' 	   => 'post',
		'public'        	     => true,
		'menu_position' 	     => 5,
    'taxonomies'           => array( 'post_tag' ),
...
add_action( 'init', 'my_custom_post_cause' );

//Add new image size for Cause featured image
if ( function_exists( 'add_image_size' ) ) {
	add_image_size( 'cause', 500, 500, true ); //(cropped)
}
